...
type
  ...
  strict private
    ...
    class function GetDuration: TMediaTime; static;
  ...
  public
    ...
    class property Duration: TMediaTime read GetDuration;
...
begin
  ...
  // Начинать нужно с небольшим отступом, что бы перепрыгнуть начальный писк/щелчок
  if ACurrentTime = 0 then
    CurrentTime := MediaTimeScale
  else
...
class function TPlayController.GetDuration: TMediaTime;
begin
  Result := FSingleSound.Duration;
end;
...
begin
  ...
  if TState.Volume = 0 then
    TVisualScheme.AssignBitmap(MainForm.SoundControl, FUNC_IDENT_MUTE)
  else
    TVisualScheme.AssignBitmap(MainForm.SoundControl, FUNC_IDENT_SOUND);
end;
...
var
  LastPlayState: TPlayState;
begin
  MainForm.TrayMenuItemPlay.Visible := true;
  MainForm.TrayMenuItemPause.Visible := false;
  ...
  TVisualScheme.AssignBitmap(MainForm.PlayControl, FUNC_IDENT_PAUSE);
  TTools.RenderPlayState(TState.PlayState);
end;
...
begin
  MainForm.TrayMenuItemMute.Visible := false;
  MainForm.TrayMenuItemUnMute.Visible := true;
...
begin
  MainForm.TrayMenuItemMute.Visible := true;
  MainForm.TrayMenuItemUnMute.Visible := false;
...
class procedure TPlayController.VolumeUp;
var
  Val: Single;
begin
  Val := TPlayController.Volume + VOLUME_STEP;
  if Val <= 1 then
    TPlayController.Volume := Val
  else
    TPlayController.Volume := 1;
end;

class procedure TPlayController.VolumeDown;
var
  Val: Single;
begin
  Val := TPlayController.Volume - VOLUME_STEP;
...
class procedure TPlayController.HeighlightChangeView;
begin
  TTools.GlowEffectActivated(
    HEIGHLIGTH_GLOW_EFFECT_IDENT,
    MainForm.ChangeViewControl,
    Assigned(PlayListForm));
end;
