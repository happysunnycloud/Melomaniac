// Юнит нужно таскать за проектом, тот который в стандартной поставке Delphi Sydney работает некорректно
...
type
  ...
  TMediaStateController = class
  private
    FMedia: TWindowsMedia;
    FWaitCompletionThread: TThread;
    FMediaState: TMediaState;
    procedure WaitCompletionThreadTerminated(Sender: TObject);
  public
    constructor Create(const AMedia: TWindowsMedia);
    { Playback thread listener }
    procedure StartWaitCompletionThread;
    procedure StopAndFreeWaitCompletionThread;

    property MediaState: TMediaState read FMediaState;
  end;

  TMediaStatesManager = class
  private
    class var FDefault: TMediaStatesManager;
  private
    FMediaStates: TDictionary<TMedia, TMediaStateController>;
    class function GetDefault: TMediaStatesManager; static;
    class destructor Destroy;
    function TryGetController(const AMedia: TWindowsMedia; var AController: TMediaStateController): Boolean;
  public
    constructor Create;
    destructor Destroy; override;

    procedure Register(const AMedia: TWindowsMedia);
    procedure Unregister(const AMedia: TWindowsMedia);
    procedure NotifyMediaPlaying(const AMedia: TWindowsMedia);
    procedure NotifyMediaStop(const AMedia: TWindowsMedia);

    function GetMediaState(const AMedia: TWindowsMedia): TMediaState;

    class property Default: TMediaStatesManager read GetDefault;
  end;
...
begin
  ...
  if InitMediaControl then
    ...
    begin
      ...
      begin
        TMediaStatesManager.Default.Register(Self);
        Exit;
      end;
  end;
  DestroyWnd;
...
begin
  TMediaStatesManager.Default.Unregister(Self);
  DestroyWnd;
  if (FMediaControl <> nil) then
    FMediaControl.StopWhenReady;
  FGraphBuilder := nil;
...
begin
  if (FMediaControl <> nil) then
    FMediaControl.Run;
  TMediaStatesManager.Default.NotifyMediaPlaying(Self);
end;
...
begin
  if (FMediaControl <> nil) then
    FMediaControl.StopWhenReady;
  TMediaStatesManager.Default.NotifyMediaStop(Self);
end;
...
begin
  Result := Succeeded(CoCreateInstance(CLSID_VideoMixingRenderer, nil, CLSCTX_INPROC, IID_IBaseFilter, FVMR)) and
    ...
    Succeeded(FVMRFilterConfig7.SetRenderingMode(VMRMode_Renderless));
end;
...
begin
  Result := Succeeded(CoCreateInstance(CLSID_VideoMixingRenderer9, nil, CLSCTX_INPROC, IID_IBaseFilter, FVMR)) and
    ...
    Succeeded(FVMRFilterConfig9.SetRenderingMode(VMR9Mode_Renderless));
end;
...
begin
  Result := TMediaStatesManager.Default.GetMediaState(Self);
end;
...
{ TMediaStateController }

constructor TMediaStateController.Create(const AMedia: TWindowsMedia);
begin
  FMediaState := TMediaState.Unavailable;
  FMedia := AMedia;
end;

procedure TMediaStateController.StartWaitCompletionThread;
var
...
begin
  ...
  begin
    ...
    if Supports(FMedia.FGraphBuilder, IMediaEvent, MediaEvent) then
    begin
...
procedure TMediaStateController.StopAndFreeWaitCompletionThread;
begin
...
procedure TMediaStateController.WaitCompletionThreadTerminated(Sender: TObject);
begin
  ...
  FMedia.FMediaControl.Pause;
end;

{ TMediaStatesManager }

constructor TMediaStatesManager.Create;
begin
  inherited;
  FMediaStates := TObjectDictionary<TMedia, TMediaStateController>.Create([doOwnsValues]);
end;

destructor TMediaStatesManager.Destroy;
begin
  FreeAndNil(FMediaStates);
  inherited;
end;

function TMediaStatesManager.TryGetController(const AMedia: TWindowsMedia; var AController: TMediaStateController): Boolean;
begin
  AController := nil;
  Result := FMediaStates.TryGetValue(AMedia, AController);
end;

class destructor TMediaStatesManager.Destroy;
begin
  FreeAndNil(FDefault);
end;

class function TMediaStatesManager.GetDefault: TMediaStatesManager;
begin
  if FDefault = nil then
    FDefault := TMediaStatesManager.Create;

  Result := FDefault;
end;

function TMediaStatesManager.GetMediaState(const AMedia: TWindowsMedia): TMediaState;
var
  Controller: TMediaStateController;
begin
  if FMediaStates.TryGetValue(AMedia, Controller) then
    Result := Controller.MediaState
  else
    Result := TMediaState.Unavailable;
end;

procedure TMediaStatesManager.NotifyMediaPlaying(const AMedia: TWindowsMedia);
var
  Controller: TMediaStateController;
begin
  if TryGetController(AMedia, Controller) then
  begin
    Controller.FMediaState := TMediaState.Playing;
    Controller.StartWaitCompletionThread;
  end;
end;

procedure TMediaStatesManager.NotifyMediaStop(const AMedia: TWindowsMedia);
var
  Controller: TMediaStateController;
begin
  if TryGetController(AMedia, Controller) then
  begin
    Controller.StopAndFreeWaitCompletionThread;
    Controller.FMediaState := TMediaState.Stopped;
  end;
end;

procedure TMediaStatesManager.Register(const AMedia: TWindowsMedia);
begin
  if FMediaStates.ContainsKey(AMedia) then
    raise Exception.Create('Media is already registered');

  FMediaStates.Add(AMedia, TMediaStateController.Create(AMedia));
end;

procedure TMediaStatesManager.Unregister(const AMedia: TWindowsMedia);
var
  Controller: TMediaStateController;
begin
  if TryGetController(AMedia, Controller) then
